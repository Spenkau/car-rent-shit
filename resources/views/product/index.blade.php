            ...
            <form method="GET" action="{{ route('products.index') }}" class="mb-10">
                ...
                <div id="extra-filters"
                    ...
                    <div>
                        ...
                        <input type="number" name="release_year" value="{{ request('release_year') }}" min="1980"
                               max="2025" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                    </div>
                    <div>
                        ...
                        <select name="gearbox_type" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                            ...
                            <option
                                value="1" {{ request('gearbox_type') === '1' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.manual')</option>
                            <option
                                value="2" {{ request('gearbox_type') === '2' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.automatic')</option>
                            <option
                                value="3" {{ request('gearbox_type') === '3' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.robotic')</option>
                            <option
                                value="4" {{ request('gearbox_type') === '4' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.variator')</option>
                            <option
                                value="5" {{ request('gearbox_type') === '5' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.dual_clutch')</option>
                            <option
                                value="6" {{ request('gearbox_type') === '6' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.semiautomatic')</option>
                            <option
                                value="7" {{ request('gearbox_type') === '7' ? 'selected' : '' }}>@lang('views.cars.gearbox_options.sequential')</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.1" name="engine_volume" value="{{ request('engine_volume') }}"
                               min="0.5" max="8.0" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                    </div>
                    <div>
                        ...
                        <select name="engine_type" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                            ...
                            <option
                                value="1" {{ request('engine_type') === '1' ? 'selected' : '' }}>@lang('views.cars.engine_options.petrol')</option>
                            <option
                                value="2" {{ request('engine_type') === '2' ? 'selected' : '' }}>@lang('views.cars.engine_options.diesel')</option>
                            <option
                                value="3" {{ request('engine_type') === '3' ? 'selected' : '' }}>@lang('views.cars.engine_options.electric')</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="drive_type" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                            ...
                            <option
                                value="1" {{ request('drive_type') === '1' ? 'selected' : '' }}>@lang('views.cars.drive_options.front')</option>
                            <option
                                value="2" {{ request('drive_type') === '2' ? 'selected' : '' }}>@lang('views.cars.drive_options.rear')</option>
                            <option
                                value="3" {{ request('drive_type') === '3' ? 'selected' : '' }}>@lang('views.cars.drive_options.electric')</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-gray-300">@lang('views.cars.power')</label>
                        <input type="number" name="power" value="{{ request('power') }}" min="50" max="2000"
                    ...
                    <div>
                        ...
                        <input type="number" name="seats_count" value="{{ request('seats_count') }}" min="1" max="50"
                               class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="price" value="{{ request('price') }}" min="1"
                               max="1000000000" class="w-full bg-gray-800 text-white px-3 py-2 rounded-md">
                    </div>
                </div>
            </form>
                ...
                @forelse ($products as $product)
                    <div
                        ...
                        <div class="car-info mb-4">
                            ...
                            <ul class="list-none p-0 text-gray-400 text-sm mt-2">
                                ...
                                <p class="text-sm text-gray-400">
                                    @lang('views.cars.gearbox'):
                                    <span class="font-medium text-green-400">
                                        @php
                                            $gearbox = \App\Enums\GearboxType::tryFrom((int) $product->settings->gearbox_type);
                                            echo $gearbox ? $gearbox->name() : __('views.cars.gearbox_options.any');
                                        @endphp
                                    </span>
                                </p>
                        ...
                        <div class="car-bottom flex justify-between items-center">
                            <div class="mt-4 mx-3">
                                <span
                                    class="car-price text-lg font-semibold text-orange-500">@lang('views.cars.price_per_day', ['price' => $product->settings->price])</span>
                            </div>
                            <div class="mt-4">
                                <a href="{{ route('products.show', $product->slug) }}"
                                   class="more-btn bg-cyan-500 text-white px-4 py-2 rounded-full text-sm font-semibold hover:bg-cyan-600 hover:scale-105 transition-all">@lang('views.cars.more')</a>
                            </div>
                        </div>
                        <div class="mt-4">
                            <button
                                ...
                                @else onclick="window.location.href='{{ route('login') }}'" @endif>
